...
unsigned long connected(adjlist* g){
     ...
     while (visitednodes!=g->n){
        ...
        for (i=1;i<k+1;i++){
            printf("Nombre d element de la composante %lu: %lu\n",i,contains[i]);
        }
...
void shortestpath(adjlist* g, unsigned long source, unsigned long destination){
     ...
     while(end>=start)      {
         ...
         for (i=0 ; i<g->cd[u+1]-g->cd[u] ;i++ ){
             v=g->adj[g->cd[u] + i];
             if (destination==v){
                printf("found shortest path with length %lu of arcs\n", depth[u]+1);
                break;
             }
             if (marked[v]==0) {
                last_marked=v;               
...
int main(int argc,char** argv){
    ...
    printf("Lower bound for grpah diameter: %lu", d );
    
    t1=time(NULL);
    
    printf("- BFSs for lower bound time = %ldh%ldm%lds\n",(t1-t3)/3600,((t1-t3)%3600)/60,((t1-t3)%60));
